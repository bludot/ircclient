...
module.exports = {
    ...
    client: function(server, username, realName, channels, webContents, port, self) {
        ...
        client.addListener('connect', function(obj) {
        ...
        client.addListener('raw', function(obj) {
            console.log(JSON.stringify(obj));
            if (obj.command && self.rpl_cmds[obj.command]) {
                obj.server = client.server;
    ...
    start: function(server, username, realName, channels, webContents, port) {
        ...
        this.clients[server] = {
            ...
            say: function(text) {
                ...
                webContents.send('client-server', {
                    action: args.action,
                    args: args.args_
        ...
        this.clients[server].client = new self.client(server, username, realName, channels, webContents, port, this.clients[server])
    },
    ...
    say: function(text) {
        var self = this;
        console.log(self.current.server);
        console.log(self.clients[self.current.server])
        self.clients[this.current.server].say(text);
    }
};
